...
def extract_block(lang, key):
    ...
    if lang_idx == -1:
        print(f"Could not find lang {lang}")
        return None
        
    key_idx = text.find(f'{key}:', lang_idx)
    if key_idx == -1:
        print(f"Could not find key {key} in {lang}")
        return None
        
    start_brace = text.find('{', key_idx)
    ...
    if end_brace != -1:
        return text[key_idx:end_brace+1]
    return None

all_langs = ['en', 'it', 'es', 'pt', 'fr', 'de']
keys_to_extract = ['nav', 'footer', 'cookie', 'contact', 'waiting', 'meta']

extracted_data = {}
for lang in all_langs:
    extracted_data[lang] = {}
    for key in keys_to_extract:
        block = extract_block(lang, key)
        if block:
            extracted_data[lang][key] = block

# Now I have all the untouched blocks.
# Let's get the home and faqs objects we generated before.
...
# Now, we write out the TS file properly.
# The top part (imports, Dictionary interface)
top_part = text[:text.find('export const TRANSLATIONS: Record<string, Dictionary> = {')]

new_ts = top_part + 'export const TRANSLATIONS: Record<string, Dictionary> = {\n'

for lang in all_langs:
    new_ts += f'  {lang}: {{\n'
    new_ts += f'    {extracted_data[lang]["nav"]},\n'
    new_ts += f'    {extracted_data[lang]["footer"]},\n'
    new_ts += f'    {extracted_data[lang]["cookie"]},\n'
    
    # Insert home
    new_ts += f'    home: {json.dumps(all_langs_data[lang][0], ensure_ascii=False, indent=6)},\n'
    
    new_ts += f'    {extracted_data[lang]["contact"]},\n'
    new_ts += f'    {extracted_data[lang]["waiting"]},\n'
    new_ts += f'    {extracted_data[lang]["meta"]},\n'
    
    # Insert faqs
    new_ts += f'    faqs: {json.dumps(all_langs_data[lang][1], ensure_ascii=False, indent=6)}\n'
    new_ts += '  },\n'
